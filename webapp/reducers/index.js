...
if (process.env.NODE_ENV !== 'production') window.__reducers__ = $root;

const actionReducers = {}, initialStates = { routing: {} };
...
function setInitialState(ctx, paths) {
  let node = ctx, lastIndex = paths.length - 1;
...
function doReduce(ctx, paths, fn, payload, meta) {
  let traversing = [], lastIndex = paths.length - 1, lastKey = paths[lastIndex], node = ctx;
